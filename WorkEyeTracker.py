...
def heartbeat():
    ...
    try:
        ...
        # ✅ MUST match backend tracker_routes.py exactly
        payload = {
            "deviceid": STATE.device_id,
            "email": CONFIG['member_email'],
            "hostname": STATE.hostname,   # optional but useful
            "osinfo": STATE.os_info       # optional but useful
        }
        ...
        if response.status_code == 200:
            ...
            if config_manager.should_sync():
                print("[HEARTBEAT] Configuration sync due, fetching...")
                config_manager.fetch_configuration()
        else:
            print(f"[HEARTBEAT] Failed: HTTP {response.status_code} → {response.text}")
...
class ActivityTracker(Thread):
    ...
    def run(self):
        while self.running:
            ...
            print(f"[IDLE CHECK] idle_time={idle_time:.1f}s threshold={CONFIG['idle_threshold']} idle={idle}")
...
class ScreenshotCapture(Thread):
    ...
    def run(self):
      ...
      while self.running:
        ...
        # wait first (so it respects interval after punch-in)
        time.sleep(interval)

        if not self.running or not STATE.is_tracking:
            continue   # ✅ now inside loop
        ...
        try:
            ...
            if screenshot_b64:
                ...
                STATE.last_screenshot_time = datetime.now()
                print(f"[SCREENSHOT] 📸 Captured at {datetime.now().strftime('%H:%M:%S')} (every {interval}s)")
        except Exception as e:
...
class MinimalUI:
    ...
    def setup_ui(self):
        ...
        self.main_frame = tk.Frame(self.root, bg='#eef2ff')
    ...
    # ---------- PUNCH IN VIEW ----------
        self.punch_in_frame = tk.Frame(card, bg='white')
        self.punch_in_frame.pack(fill=tk.BOTH, expand=True, padx=30, pady=30)

        tk.Label(
            self.punch_in_frame,
            text="WorkEye Tracker",
        ...
        self.punch_in_btn.pack(pady=25, fill='x')


    # ---------- PUNCH OUT VIEW ----------
        self.punch_out_frame = tk.Frame(card, bg='white')
        ...
        self.tracking_label.pack(pady=(10, 35))
        ...
        self.punch_out_btn.pack(fill='x', padx=30, pady=(0, 25))
    ...
    def show_punch_out_view(self):
        ...
        self.root.title("WorkEye Tracker")

    # prevent overflow → wrap long emails
        self.tracking_label.config(
        text=f"Tracking Active\n{email}",
        wraplength=320,        # ⭐ fixes overflow
        justify="center"
    )
    ...
    def handle_punch_out(self):
        ...
        if self.config_thread:
           self.config_thread.running = False

        if self.screenshot_thread:
           self.screenshot_thread.running = False
