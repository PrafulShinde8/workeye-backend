...
========================================
FIXES:
  1. idle_threshold = 60s (was 180s) - goes idle after 1 min no input
  2. upload_interval = 15s (was 30s) - status updates reach backend faster
  3. All thread/socket/lock fixes from previous version retained
"""

import os, sys, time, json, base64, ctypes, psutil
...
import requests
from threading import Thread, Lock
import logging, socket, getpass, platform
import tkinter as tk
from tkinter import messagebox
import uuid
...
CONFIG = {
    ...
    'upload_interval': 15,      # was 30s
    'idle_threshold': 60,       # was 180s — now 60s = 1 minute
    'heartbeat_interval': 60,
    'device_id': None,
    'member_email': None,
}
...
class ConfigurationManager:
    ...
    def fetch_configuration(self):
        ...
        try:
            ...
            url = f"{CONFIG['backend_url']}/api/tracker/configuration"
            params = {'device_id': STATE.device_id if hasattr(STATE, 'device_id') else 'unknown'}
            response = requests.get(url, headers={'X-Tracker-Token': CONFIG['tracker_token']},
                                    params=params, timeout=10)
            if response.status_code == 200:
                data = response.json()
                if data.get('success'):
                    screenshot_min = data.get('screenshot_interval_minutes', 5)
                    idle_min = data.get('idle_timeout_minutes', 1)
                    CONFIG['screenshot_interval'] = screenshot_min * 60
                    CONFIG['idle_threshold'] = max(30, idle_min * 60)
                    CONFIG['office_start_time'] = data.get('office_start_time', '09:00:00')
                    CONFIG['office_end_time'] = data.get('office_end_time', '18:00:00')
                    CONFIG['working_days'] = data.get('working_days', [1, 2, 3, 4, 5])
                    self.last_sync = time.time()
                    print(f"✅ Config: screenshot={screenshot_min}min, idle={CONFIG['idle_threshold']}s")
                    return True
        except Exception as e:
            print(f"[CONFIG] {e}")
        finally:
...
class GlobalState:
    ...
    def add_time(self, seconds, is_active, is_idle, is_locked):
        with self.lock:
            self.total_seconds += seconds
            if is_locked: self.locked_seconds += seconds
            elif is_idle: self.idle_seconds += seconds
            elif is_active: self.active_seconds += seconds
    ...
    def get_payload(self, include_screenshot=False):
        with self.lock:
            payload = {
                "deviceid": self.device_id, "username": self.username,
                "email": CONFIG.get('member_email', ''), "hostname": self.hostname,
                "osinfo": self.os_info, "timestamp": datetime.now().isoformat(),
                "sessionstart": self.session_start.isoformat(),
                "lastactivity": self.last_activity_time.isoformat(),
                "totalseconds": round(self.total_seconds, 2),
                "activeseconds": round(self.active_seconds, 2),
                "idleseconds": round(self.idle_seconds, 2),
                "lockedseconds": round(self.locked_seconds, 2),
                "idlefor": round(self.idle_for, 2),
                "isidle": self.is_idle, "locked": self.is_locked,
                "mouseactive": self.mouse_active, "keyboardactive": self.keyboard_active,
                "currentwindow": self.current_window, "currentprocess": self.current_process,
                "windowsopened": self.windows_opened[:], "browserhistory": [],
            }
            if include_screenshot and self.latest_screenshot_b64:
                payload["screenshot"] = self.latest_screenshot_b64
            return payload
    ...
    def reset_session(self):
        with self.lock:
            self.total_seconds = self.active_seconds = self.idle_seconds = self.locked_seconds = 0.0
            self.is_idle = self.is_locked = False
            self.idle_for = 0.0
            self.mouse_active = self.keyboard_active = False
            self.session_start = datetime.now()
            self.windows_opened = []
            self.latest_screenshot_b64 = None
...
def is_screen_locked():
    try:
        ...
        try: return win32gui.GetForegroundWindow() == 0
...
def get_active_window_info():
    try:
        hwnd = win32gui.GetForegroundWindow()
        if hwnd == 0: return "Desktop", "explorer.exe"
        title = win32gui.GetWindowText(hwnd)
        _, pid = win32process.GetWindowThreadProcessId(hwnd)
        return title or "Unknown", psutil.Process(pid).name()
    except Exception:
        return "Unknown", "unknown.exe"
...
def capture_screenshot():
    try:
        ...
        img.save(buf, format="JPEG", quality=75)
        try:
            sd = os.path.join(CONFIG['config_dir'], 'screenshots')
            os.makedirs(sd, exist_ok=True)
            with open(os.path.join(sd, datetime.now().strftime('%Y%m%d_%H%M%S') + '.jpg'), 'wb') as f:
...
def verify_member(email):
    try:
        if not CONFIG.get('tracker_token'):
            return False, None, "Token not configured. Download tracker from admin dashboard."
        url = f"{CONFIG['backend_url']}/tracker/verify-member"
        payload = {"email": email.lower().strip(), "deviceid": STATE.device_id,
                   "hostname": STATE.hostname, "osinfo": STATE.os_info}
        headers = {'Content-Type': 'application/json', 'X-Tracker-Token': CONFIG['tracker_token']}
        response = requests.post(url, json=payload, headers=headers, timeout=15)
        if response.status_code == 200:
            data = response.json()
            if data.get('success'):
                CONFIG['member_email'] = email.lower().strip()
                member = data.get('member', {})
                print(f"[VERIFY] ✅ {member.get('name')} ({member.get('id')})")
                return True, member, "OK"
            return False, None, data.get('error', 'Verification failed')
        try: error = response.json().get('error', f"HTTP {response.status_code}")
        except Exception: error = f"HTTP {response.status_code}"
        return False, None, error
    except requests.exceptions.Timeout:
        return False, None, "Timeout."
    except requests.exceptions.ConnectionError:
        return False, None, "Cannot connect to backend."
    except Exception as e:
        return False, None, str(e)
...
def punch_in():
    try:
        if not CONFIG.get('member_email') or not CONFIG.get('tracker_token'):
            return False, "Not configured"
        r = requests.post(f"{CONFIG['backend_url']}/tracker/punch-in",
                          json={"email": CONFIG['member_email'], "deviceid": STATE.device_id},
                          headers={'Content-Type': 'application/json', 'X-Tracker-Token': CONFIG['tracker_token']},
                          timeout=15)
        if r.status_code == 200:
            d = r.json()
            print(f"[PUNCH-IN] ✅ {d.get('message')}")
            return True, d.get('message', 'Punched in')
        try: error = r.json().get('error', 'Failed')
        except Exception: error = f"HTTP {r.status_code}"
        return False, error
    except Exception as e:
        return False, str(e)
...
def punch_out():
    try:
        if not CONFIG.get('member_email') or not CONFIG.get('tracker_token'):
            return False, "Not configured"
        r = requests.post(f"{CONFIG['backend_url']}/tracker/punch-out",
                          json={"email": CONFIG['member_email'], "deviceid": STATE.device_id},
                          headers={'Content-Type': 'application/json', 'X-Tracker-Token': CONFIG['tracker_token']},
                          timeout=15)
        if r.status_code == 200:
            d = r.json()
            print(f"[PUNCH-OUT] ✅ {d.get('message')}")
            return True, d.get('message', 'Punched out')
        try: error = r.json().get('error', 'Failed')
        except Exception: error = f"HTTP {r.status_code}"
        return False, error
    except Exception as e:
...
def upload_data():
    try:
        if not CONFIG.get('member_email') or not CONFIG.get('tracker_token'):
            return False
        payload = STATE.get_payload(include_screenshot=STATE.latest_screenshot_b64 is not None)
        r = requests.post(f"{CONFIG['backend_url']}/tracker/upload", json=payload,
                          headers={'Content-Type': 'application/json',
                                   'X-Tracker-Token': CONFIG['tracker_token']}, timeout=15)
        if r.status_code == 200:
            data = r.json()
            if data.get('code') == 'NOT_PUNCHED_IN':
                print("[UPLOAD] ⚠️ Not punched in — stopping")
                STATE.is_tracking = False
                return False
            status = "IDLE" if STATE.is_idle else ("LOCKED" if STATE.is_locked else "ACTIVE")
            print(f"[UPLOAD] ✅ {status} idle_for={STATE.idle_for:.0f}s")
            STATE.reset_for_upload()
            return True
        print(f"[UPLOAD] ⚠️ HTTP {r.status_code}")
        return False
    except Exception as e:
        print(f"[UPLOAD] ❌ {e}")
        return False
...
def heartbeat():
    try:
        if not CONFIG.get('member_email') or not CONFIG.get('tracker_token'):
            return
        r = requests.post(f"{CONFIG['backend_url']}/tracker/heartbeat",
                          json={"deviceid": STATE.device_id, "email": CONFIG['member_email'],
                                "hostname": STATE.hostname, "osinfo": STATE.os_info},
                          headers={'X-Tracker-Token': CONFIG['tracker_token']}, timeout=5)
        if r.status_code == 200:
            print("[HEARTBEAT] ❤️")
            if config_manager.should_sync():
                config_manager.fetch_configuration()
...
class ActivityTracker(Thread):
    ...
    def run(self):
        print(f"[ACTIVITY] Started — idle after {CONFIG['idle_threshold']}s no input")
        while self.running:
            if not STATE.is_tracking:
                time.sleep(CONFIG['activity_check_interval'])
                continue
            ...
            idle_time = get_idle_time()
            locked = is_screen_locked()
            idle = (idle_time >= CONFIG['idle_threshold']) and not locked
            with STATE.lock:
                STATE.is_idle = idle
                STATE.is_locked = locked
                STATE.idle_for = idle_time
            if not locked:
                ...
                if not idle:
                    ...
                    with STATE.lock:
                        STATE.last_activity_time = datetime.now()
                        STATE.keyboard_active = (idle_time < 2.0)
                else:
                    with STATE.lock:
                        STATE.mouse_active = False
                ...
                with STATE.lock:
                    STATE.mouse_active = False
                    STATE.keyboard_active = False
            STATE.add_time(elapsed, not idle and not locked, idle, locked)
            time.sleep(CONFIG['activity_check_interval'])
...
class DataUploader(Thread):
    ...
    def run(self):
        print(f"[UPLOADER] Started — every {CONFIG['upload_interval']}s")
        while self.running:
            interval = CONFIG['upload_interval']
            elapsed = 0
            while elapsed < interval and self.running:
                time.sleep(min(2, interval - elapsed))
                elapsed += 2
                if not STATE.is_tracking: break
            if not self.running or not STATE.is_tracking: break
            upload_data()
...
class HeartbeatSender(Thread):
    ...
    def run(self):
        while self.running:
            elapsed = 0
            while elapsed < CONFIG['heartbeat_interval'] and self.running:
                time.sleep(min(2, CONFIG['heartbeat_interval'] - elapsed))
                elapsed += 2
                if not STATE.is_tracking: break
            if not self.running or not STATE.is_tracking: break
            heartbeat()
...
class ScreenshotCapture(Thread):
    ...
    def run(self):
        while self.running:
            ...
            elapsed = 0
            while elapsed < interval and self.running and STATE.is_tracking:
                st = min(30, interval - elapsed)
                time.sleep(st)
                elapsed += st
                new_interval = CONFIG['screenshot_interval']
            ...
            try:
                ...
                if b64:
                    with STATE.lock:
                        ...
                        STATE.last_screenshot_time = datetime.now()
                    print(f"[SCREENSHOT] 📸 {datetime.now().strftime('%H:%M:%S')}")
            except Exception as e:
                print(f"[SCREENSHOT] ❌ {e}")
...
class MinimalUI:
    def __init__(self):
        ...
        self.root.configure(bg='#f8fafc')
        self.email_var = tk.StringVar()
        self.activity_thread = self.upload_thread = self.heartbeat_thread = None
        self.config_thread = self.screenshot_thread = None
        self.setup_ui()
        self.load_saved_email()
    ...
    def setup_ui(self):
        ...
        self.punch_in_frame = tk.Frame(self.main_frame, bg='#f8fafc')
        self.punch_in_frame.pack(fill=tk.BOTH, expand=True)
        tk.Label(self.punch_in_frame, text="Enter your email to Punch IN",
                 font=('Arial', 16, 'bold'), bg='#f8fafc', fg='#1e293b').pack(pady=(0, 30))
        tk.Label(self.punch_in_frame, text="Email:", font=('Arial', 12),
                 bg='#f8fafc', fg='#475569').pack(anchor='w')
        self.email_entry = tk.Entry(self.punch_in_frame, textvariable=self.email_var,
                                    font=('Arial', 12), width=30, relief='solid', bd=1)
        self.email_entry.pack(pady=5, fill='x')
        self.email_entry.focus()
        self.punch_in_btn = tk.Button(self.punch_in_frame, text="PUNCH IN",
                                      font=('Arial', 18, 'bold'), bg='#059669', fg='white',
                                      relief='flat', height=2, command=self.handle_punch_in, cursor='hand2')
        self.punch_in_btn.pack(pady=30, fill='x')
        self.punch_out_frame = tk.Frame(self.main_frame, bg='#f8fafc')
        self.tracking_label = tk.Label(self.punch_out_frame, text="",
                                       font=('Arial', 16, 'bold'), bg='#f8fafc', fg='#059669')
        self.tracking_label.pack(pady=(20, 50))
        self.punch_out_btn = tk.Button(self.punch_out_frame, text="PUNCH OUT",
                                       font=('Arial', 18, 'bold'), bg='#dc2626', fg='white',
                                       relief='flat', height=3, command=self.handle_punch_out, cursor='hand2')
        self.punch_out_btn.pack(pady=20, fill='both', expand=True, padx=20)
        self.root.bind('<Return>', lambda e: self.handle_punch_in() if not STATE.is_tracking else None)
    ...
    def load_saved_email(self):
        try:
            if os.path.exists(CONFIG['config_file']):
                with open(CONFIG['config_file'], 'r') as f:
                    d = json.load(f)
                    if d.get('member_email'):
                        self.email_var.set(d['member_email'])
        except Exception: pass

    def save_email(self):
        try:
            with open(CONFIG['config_file'], 'w') as f:
                json.dump({'member_email': CONFIG.get('member_email'), 'device_id': STATE.device_id}, f)
        except Exception: pass
    ...
    def _stop_all_threads(self):
        STATE.is_tracking = False
        for t in [self.activity_thread, self.upload_thread, self.heartbeat_thread,
                  self.config_thread, self.screenshot_thread]:
    ...
    def handle_punch_in(self):
        ...
        try:
            self.punch_in_btn.config(state='disabled', text='Verifying...')
            self.root.update()
            success, member, msg = verify_member(email)
            if not success:
                messagebox.showerror("Error", msg or "Verification failed")
                self.punch_in_btn.config(state='normal', text='PUNCH IN')
                return
            self.punch_in_btn.config(text='Loading config...')
            self.root.update()
            ...
            self.punch_in_btn.config(text='Punching in...')
            self.root.update()
            punch_success, punch_msg = punch_in()
            if not punch_success:
                messagebox.showerror("Error", f"Failed to punch in: {punch_msg}")
                self.punch_in_btn.config(state='normal', text='PUNCH IN')
                return
            STATE.is_tracking = True
            STATE.punch_in_time = STATE.session_start = datetime.now()
            self.activity_thread = ActivityTracker()
            self.upload_thread = DataUploader()
            self.heartbeat_thread = HeartbeatSender()
            self.config_thread = ConfigurationFetcher()
            ...
            self.save_email()
            self.show_punch_out_view()
            print(f"✅ TRACKING: {email} | idle={CONFIG['idle_threshold']}s | upload={CONFIG['upload_interval']}s")
        except Exception as e:
    ...
    def handle_punch_out(self):
        if not messagebox.askyesno("Confirm", "Stop tracking and punch out?"):
            return
        self._stop_all_threads()
        time.sleep(0.5)
        success, msg = punch_out()
        if success: messagebox.showinfo("Punched Out", msg)
        else: messagebox.showwarning("Punched Out", f"Stopped.\n{msg}")
        STATE.reset_session()
        self.email_entry.config(state='normal')
        self.punch_in_btn.config(state='normal', text="PUNCH IN")
        self.show_punch_in_view()
    ...
    def on_closing(self):
        if STATE.is_tracking:
            if not messagebox.askyesno("Confirm Exit", "Exit will punch you out. Continue?"):
                return
            self._stop_all_threads()
            time.sleep(0.5)
            punch_out()
...
if __name__ == '__main__':
    print("=" * 60)
    print("WORK TRACKER")
    print(f"Device: {STATE.device_id} | Idle after: {CONFIG['idle_threshold']}s")
